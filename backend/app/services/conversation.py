...
from app.services.llm import (
    ...
    detect_language_switch, detect_language_llm,
    get_live_opportunities, get_fund_readiness_plan,
    match_with_explanation, should_search_web,
    should_get_readiness_plan, LANGUAGE_NAMES,
)
...
async def handle_incoming_message(
    ...
    if new_lang and new_lang != profile.language:
        ...
        await upsert_profile(profile)
        msg = LANGUAGE_SWITCH_MESSAGES.get(new_lang, f"Switched to {LANGUAGE_NAMES.get(new_lang, 'the new language')}.")
    ...
    audio = await _maybe_tts(response, profile.language, is_voice)
    return {"text_response": response, "audio_path": audio}
...
async def _process(profile: SMEProfile, user_text: str, is_voice: bool) -> str:
    ...
    if session:
        ...
        resume = f"\n\nWhen you're ready, let's continue your document check. {build_document_question(session['current_doc'], lang)}"
...
async def _start_document_flow(profile: SMEProfile, opp_id: str, opp_name: str) -> str:
    """Start the document walk-through for a specific opportunity."""
    lang = profile.language
    ...
    _doc_flow_sessions[profile.phone_number] = {
        "opp_id": opp_id,
        "opp_name": opp_name,
    ...
    intros = {
        ...
        "yo": f"Jeki mo ran yin lowo lati mura lati fi ohun elo sile fun *{opp_name}*.\n\nMo o lo nipase awon iwe ipo kookan pelu yin. So beeni tabi rara fun ookan.\n\n",
        "ha": f"Bari in taimake ka shirya don neman *{opp_name}*.\n\nZa mu bi ka game da kowannen takardu ɗaya bayan ɗaya. Ka amsa eh ko a'a ga kowanne.\n\n",
    ...
    return intro + build_document_question(next_doc, lang)


async def _handle_doc_yes(profile: SMEProfile, session: dict) -> str:
    """User has the current document."""
    lang = profile.language
...
async def _handle_doc_no(profile: SMEProfile, session: dict) -> str:
    """User does not have the current document."""
    lang = profile.language
    opp_id = session["opp_id"]
    opp_name = session["opp_name"]
    current = session["current_doc"]
    ...
    # Move to next doc after showing instructions
    next_doc = get_next_unchecked_document(profile.phone_number, opp_id)
    if next_doc:
        session["current_doc"] = next_doc
        await _set_doc_session(profile.phone_number, session)
...
def _help_menu(lang: str) -> str:
    menus = {
        "en": (
            ...
            "Explain CAC, BVN, TIN, bank statements   anything Nigerian business\n\n"
            "*Commands:*\n"
...
async def _handle_profiled(profile: SMEProfile, user_text: str) -> str:
    ...
    # Detect profile update intents
    update_triggers = ["my revenue", "i registered", "i got cac", "i now have",
                       "i moved", "changed my", "actually i", "update my"]
    if any(t in lower for t in update_triggers):
        return await _handle_profile_update(profile, user_text, lang)

    history = await format_history_for_llm(profile.phone_number)
    return await chat_with_sme(history, user_text, lang) or "Having a small issue. Try again in a moment."


async def _handle_profile_update(profile: SMEProfile, user_text: str, lang: str) -> str:
    """Detect and apply a single-field profile update."""
    history = await format_history_for_llm(profile.phone_number)
    response = await chat_with_sme(history, user_text, lang)
    ...
    if extracted:
        ...
        if updated:
            msgs = {
                "en": f"Updated your profile. Let me re-check your matches with the new information.\n\n",
                "pcm": f"I don update your profile. Make I check your matches again.\n\n",
                "fr": f"Profil mis à jour. Voici vos nouvelles correspondances.\n\n",
            }
            prefix = msgs.get(lang, msgs["en"])
            result = await match_with_explanation(updated.to_summary(), get_opportunities_text(), lang)
            return prefix + (result or format_opportunities_for_whatsapp(get_matched_opportunities(updated)))

    return response or "Got it. What else can I help with?"


# ─────────────────────────────────────────────────────────────────────────────
# OPPORTUNITY SELECTION + DOCUMENT FLOW
# ─────────────────────────────────────────────────────────────────────────────
...
async def _handle_opportunity_selection(profile: SMEProfile, index: int) -> str:
    ...
    readiness_score, missing = calculate_readiness_score(profile.phone_number, opp_id)
    ...
    response = (
        f"*{opp_name}* ({score}% match{readiness_tag})\n\n"
    ...
    check_offers = {
        "en": "Want me to check which of these documents you already have and help you get the rest? Reply *check documents* to start.",
        "pcm": "You wan make I check which documents you get and help you with the ones wey you no get? Say *check documents*.",
        "yo": "E feja ki mo wa mọ ewo ninu awon iwe ipo ti e ni tẹlẹ ki mo si ran yin lowo lati gba awon to ku? So *check documents*.",
        "fr": "Voulez-vous que je vérifie quels documents vous avez déjà et vous aide à obtenir les autres? Dites *vérifier documents*.",
    }
    ...
    await upsert_profile(profile)
    await save_message(profile.phone_number, "system", f"[VIEWED_OPPORTUNITY:{opp_id}:{opp_name}]")
    return response
...
async def _handle_document_request(profile: SMEProfile, user_text: str) -> str:
    ...
    if "check document" in lower or "verify document" in lower or "vérifier document" in lower:
        ...
        if last_opp_id:
            return await _start_document_flow(profile, last_opp_id, last_opp_name)
        else:
            return "Which opportunity do you want to check documents for? First reply with a number from your matches list, then ask me to check documents."
...
async def _handle_application_confirmation(profile: SMEProfile) -> str:
    ...
    if last_opp_id and last_opp_name:
        ...
        msgs = {
            "en": f"Logged your application for *{last_opp_name}*. I will remind you as the deadline approaches.\n\nType *my applications* to see your full tracker.",
            "pcm": f"I don log your application for *{last_opp_name}*. I go remind you before deadline.\n\nType *my applications* to check.",
            "fr": f"Candidature pour *{last_opp_name}* enregistrée. Je vous rappellerai avant la date limite.",
        }
...
async def _handle_applications(profile: SMEProfile) -> str:
    ...
    for app in apps[:10]:
        ...
        # Find deadline for this opportunity
